...
def test_execute_dag_incremental(tmp_path, mocker):
    ...
    dag = nx.DiGraph()
    dag.add_node("proj.data.inc_model", 
        type="incremental", 
    ...
    assert any("INSERT INTO data_inc_model SELECT * FROM source WHERE date > '2023-01-01'" in call for call in calls)
...
def test_execute_dag_incremental_full_refresh(tmp_path, mocker):
    ...
    assert any("CREATE TABLE data_inc_model AS SELECT * FROM source" in call for call in calls)
...
def test_execute_dag_assertion_failure(tmp_path, mocker):
    ...
    dag = nx.DiGraph()
    dag.add_node("proj.data.assert_model", 
        type="assertion", 
