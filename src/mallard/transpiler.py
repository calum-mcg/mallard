...
def __log_debug(hypothesis_id, message, data):
    ...
    try:
        with open(
            "/Users/calummcguicken/Documents/github/mallard/.cursor/debug-4f7a57.log",
            "a",
        ) as f:
            f.write(
                json.dumps(
    ...
    try:
        ...
        if isinstance(node, exp.FarmFingerprint):
            return exp.Anonymous(
                this="hash",
                expressions=[
                    e.transform(_transform_ast, copy=False) for e in node.expressions
                ],
            )
        ...
        if (
            ...
            and node.name
            and node.name.upper() == "FARM_FINGERPRINT"
        ):
            return exp.Anonymous(
                this="hash",
                expressions=[
        ...
        if isinstance(node, exp.ArrayLast):
            return exp.Anonymous(
                this="list_last",
                expressions=[node.this.transform(_transform_ast, copy=False)],
            )
        ...
        if isinstance(node, exp.Table):
            ...
            if (
                node.args.get("db")
                and node.args["db"].name.upper() == "INFORMATION_SCHEMA"
            ):
                return node
            ...
            if db_val and this_val:
                ...
                new_name = "_".join(parts).replace("-", "_")
                return exp.Table(
                    this=exp.Identifier(this=new_name, quoted=False),
        ...
        if isinstance(node, exp.Identifier):
            name_str = node.name
            if name_str and (
                "PRIVATE_UDFS" in name_str.upper()
                or "PUBLIC_UDFS" in name_str.upper()
                or "PRIVATE_SUBSCRIPTIONS" in name_str.upper()
            ):
                parts = (
                    name_str.replace("`", "")
                    .replace('"', "")
                ...
                if "enriched_subscriptions_intermediate_transformation" in name_str:
                    ...
                    new_name = (
                        "private_subscriptions_monthly_user_cohort_retention_fts_eop"
                    )
        ...
        if isinstance(node, exp.Anonymous):
            if node.name:
                if node.name.upper() == "FARM_FINGERPRINT":
                    return exp.Anonymous(
                        this="hash",
                        expressions=[
                            e.transform(_transform_ast, copy=False)
                    ...
                    if len(node.expressions) == 3:
                        ...
                        if isinstance(date_part, exp.Literal):
                            ...
                            return exp.Anonymous(
                                this="date_diff",
                                expressions=[
                                    exp.Identifier(this=date_part_str, quoted=False),
                                    node.expressions[1].transform(
                                        _transform_ast, copy=False
                                    ),
                                    node.expressions[2].transform(
                                        _transform_ast, copy=False
                                    ),
                                ],
                            )
            ...
            func_name = (
                node.this.name if isinstance(node.this, exp.Identifier) else node.name
            )
        ...
        if isinstance(node, exp.In):
            if node.args.get("unnest"):
                ...
                if isinstance(unnest_node, exp.Unnest):
                    expr = (
                        unnest_node.expressions[0]
                        if unnest_node.expressions
                        else exp.Null()
                    )
                    return exp.Anonymous(
                        this="list_contains", expressions=[expr, node.this]
                    )
        ...
        if isinstance(node, exp.Unnest):
            ...
            if expr:
                ...
                if offset:
                    ...
                    zero_indexed = exp.Sub(
                        this=subscripts, expression=exp.Literal.number(1)
                    )
                    offset_name = (
                        offset.name if hasattr(offset, "name") else offset.alias
                    )
                ...
                if alias:
                    ...
                    subquery.set(
                        "alias",
                        exp.TableAlias(
                            this=exp.Identifier(this=subquery_alias, quoted=False)
                        ),
                    )
                else:
                    pass

                return exp.Lateral(this=subquery)
...
def _duckdb_select_struct(self, expression):
    if expression.args.get("kind") == "STRUCT":
        ...
        expr_clone.expressions.append(
            exp.alias_(exp.Literal.number(1), "_dummy_field_to_prevent_empty_struct")
        )

        subquery = exp.Subquery(
            this=expr_clone,
            alias=exp.TableAlias(
                this=exp.Identifier(this="__struct_alias", quoted=False)
            ),
        )

        wrapper_select = exp.Select(
            expressions=[
                exp.Column(this=exp.Identifier(this="__struct_alias", quoted=False))
            ]
        ).from_(subquery)
