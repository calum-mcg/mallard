...
@app.command("fetch")
def fetch(
    vars: str = typer.Option(None, "--vars", help="Inject variables into compilation, e.g. date_id=2023-01-01"),
):
    """
...
@app.command("run")
def run(
    select: str = typer.Option(None, "--select", help="Run specific models, e.g. +stg_orders+"),
    tags: str = typer.Option(None, "--tags", help="Run models matching specified tags, e.g. core,hourly"),
    upstream: bool = typer.Option(False, "--upstream", help="Include upstream dependencies for selected models"),
    downstream: bool = typer.Option(False, "--downstream", help="Include downstream dependents for selected models"),
    refresh_sources: bool = typer.Option(False, "--refresh-sources", help="Invalidate local DuckDB cache and re-fetch BigQuery partitions"),
    full_refresh: bool = typer.Option(False, "--full-refresh", help="Force full rebuild of incremental models"),
    vars: str = typer.Option(None, "--vars", help="Inject variables into compilation, e.g. date_id=2023-01-01"),
):
    """
    ...
    console.print(f"[bold green]Selected {len(selected_nodes)} models to run.[/bold green]")
...
@app.command("iterate")
def iterate(
    var: str = typer.Option(..., "--var", help="Variable name to iterate over, e.g. date_id"),
    values: str = typer.Option(..., "--values", help="Comma-separated values to iterate, e.g. 2023-01-01,2023-01-02"),
    select: str = typer.Option(None, "--select", help="Run specific models, e.g. +stg_orders+"),
    tags: str = typer.Option(None, "--tags", help="Run models matching specified tags, e.g. core,hourly"),
    upstream: bool = typer.Option(False, "--upstream", help="Include upstream dependencies for selected models"),
    downstream: bool = typer.Option(False, "--downstream", help="Include downstream dependents for selected models"),
    refresh_sources: bool = typer.Option(False, "--refresh-sources", help="Invalidate local DuckDB cache and re-fetch BigQuery partitions before executing"),
    full_refresh: bool = typer.Option(False, "--full-refresh", help="Force full rebuild of incremental models"),
):
    """
    ...
    for val in val_list:
        ...
        console.print(f"[bold blue]Compiling Dataform project for {var}={val}...[/bold blue]")
